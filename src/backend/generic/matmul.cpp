...
namespace athena::backend::generic {
    ...
    athena::core::Tensor *matmulf(bool aTransp, athena::core::Tensor *a, bool bTransp, athena::core::Tensor *b) {
    ...
    athena::core::Tensor *matmuld(bool aTransp, athena::core::Tensor *a, bool bTransp, athena::core::Tensor *b) {
        ...
        cblas_dgemm(C_ORDER, // Specifies row-major (C) or column-major (Fortran) data ordering
                    aTransp ? TRANS : NO_TRANS, // Specifies whether to transpose matrix A
                    bTransp ? TRANS : NO_TRANS, // Specifies whether to transpose matrix B
                    static_cast<const int>(a->getShape().dim(0)), // Number of rows in matrices A and C.
                    static_cast<const int>(b->getShape().dim(1)), // Number of columns in matrices B and C
                    static_cast<const int>(a->getShape().dim(1)), // Number of columns in matrix A; number of rows in matrix B
                    1, // Scaling factor for the product of matrices A and B
                    af, // Matrix A
                    static_cast<const int>(a->getShape().dim(0)), // The size of the first dimention of matrix A; if you are passing a matrix A[m][n], the value should be m
                    bf, // Matrix B
                    static_cast<const int>(b->getShape().dim(0)), // The size of the first dimention of matrix B; if you are passing a matrix B[m][n], the value should be m
                    1, // Scaling factor for matrix C
                    cf, // Matrix C
                    static_cast<const int>(a->getShape().dim(0))); // The size of the first dimention of matrix C; if you are passing a matrix C[m][n], the value should be m
